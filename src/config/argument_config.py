...
class ArgumentConfig(PrintableConfig):
    ########## batch arguments ##########
    flag_process_batch: bool = True
    batch_source_dir: Annotated[str, tyro.conf.arg(aliases=["-sd"])] = '/data2/wcc/rodinhd_data/portrait3d_data/'
    batch_driving_dir: Annotated[str, tyro.conf.arg(aliases=["-dd"])] = '/data1/wcc/LivePortrait/data/driving'
    batch_output_dir: Annotated[str, tyro.conf.arg(aliases=["-od"])] = '/data2/wcc/rodinhd_data/portrait3dexp_data/'
    ...
    render: Annotated[str, tyro.conf.arg(aliases=["-r"])] = make_abs_path('') # Nyte's add: replace triplane's render result.
    ...
    animation_region: Literal["exp", "pose", "lip", "eyes", "all"] = "exp" # the region where the animation was performed, "exp" means the expression, "pose" means the head pose, "all" means all regions
